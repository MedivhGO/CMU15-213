...
/*
    ...
    在非规格化的数中
    阶码的值是 E = 1 - bias, 这种方式提供了一种从非规格化值平滑转换到规格化值的方法.
    M = f = 0.fn-1fn-2...f1f0
*/
...
int floatFloat2Int(unsigned uf) {
  ...
  */
  ...
  if (E < 0) return 0; // 所表示的是一个小于1的数,所以转换成int型,直接返回0. exp 0或255 才有特殊含义与E无关
...
unsigned floatPower2(int x) {
    ...
    E 为 -126
    x > 127 这个数太大了,返回无穷
    -126 <= x <= 127 //可以使用一个规范化的数进行表示
    -148 <= x < -126 可以使用一个非规格化数表示,也就是exp为全0.这时E为固定值-126,再有frac中前导0来记这个数的负值数量级.
    如果 x < -126 -23 + 1 = -148那么这个数就太小了,不能表示了.就返回0
    */
    if (x > 127) {
        return 0xFF << 23; // 也就是0111111110000.... 就是exp为全1的frac为全0所表示的无穷.
    } else if (x < -148) {
        return 0;
    } else if (x >= -126) {
        int exp = x + 127; // 给出E求 exp
        return (exp << 23);
    } else { //    -148 <= x < -126
        int t = x + 148; //当 x = -148的时候得到最小的非规格化数,也就是frac里除了最后一位都是0
        return (1 << t); // 000000...00000001
    }
}
